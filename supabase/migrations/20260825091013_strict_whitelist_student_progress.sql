-- Strict whitelist for student progress: a cohort with zero rows in
-- cohort_assignments no longer means "all lessons". If enrollments.cohort_id
-- is set, the lesson must exist in cohort_assignments for that cohort.
...
COMMENT ON FUNCTION public.get_my_student_progress() IS
  'Строки прогресса (тесты и задания) для текущего ученика (auth.uid()). Если enrollment.cohort_id задан, урок должен быть в cohort_assignments (строгий whitelist: пустой список = нет доступа).';
